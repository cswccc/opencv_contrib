...
QBAR_RESULT QBar::Decode(Mat &image)
{
    // Mat image(height, width, CV_8UC1, imageData);
    return qbarDecode_->Decode(image);
}

std::vector<QBAR_RESULT> QBar::ScanImage(Mat &image)
{
    // Mat image(height, width, CV_8UC1, imageData);
...
int QBarGen::MakeRoundQRCode(const QBAR_IMAGE& code, const QBAR_ENCODE_CONFIG& config, QBarImageInfo& buf)
{
    ...
    {
        cvEyeImage = imread(config.personal_param_.eye_path_, CV_LOAD_IMAGE_UNCHANGED);
        if (cvEyeImage.data == nullptr) return -1;
        cvModuleImage = imread(config.personal_param_.module_path_, CV_LOAD_IMAGE_UNCHANGED);
        if (cvModuleImage.data == nullptr) return -1;
    }
    else if (config.personal_param_.mode_  == MODE_PERSONAL_BLACK)
    {
        cvEyeImage = imread(config.personal_param_.black_eye_path_, CV_LOAD_IMAGE_UNCHANGED);
        if (cvEyeImage.data == nullptr) return -1;
        cvModuleImage = imread(config.personal_param_.black_module_path_, CV_LOAD_IMAGE_UNCHANGED);
        if (cvModuleImage.data == nullptr) return -1;
    }
    else return -1;
#else
    if (config.personal_param_.mode_ == MODE_PERSONAL_WHITE)
    {
        Mat cvEyeImage(config.personal_param_.eye_image_.height_,
                           config.personal_param_.eye_image_.width_, CV_8UC4,
                           config.personal_param_.eye_image_.image_data_);
        if (cvEyeImage.data == nullptr) return -1;
        
        Mat cvModuleImage(config.personal_param_.module_image_.height_,
    ...
    {
        Mat cvEyeImage(config.personal_param_.black_eye_image_.height_,
                           config.personal_param_.black_eye_image_.width_, CV_8UC4,
                           config.personal_param_.black_eye_image_.image_data_);
        if (cvEyeImage.data == nullptr) return -1;
        Mat cvModuleImage(config.personal_param_.black_module_image_.height_,
...
int QBarGen::MakePersonQRCode(const QBAR_IMAGE& code, const QBAR_ENCODE_CONFIG& config, QBarImageInfo& buf)
{
    ...
    if (config.personal_param_.mode_ == MODE_PERSONAL_WHITE)
    {
        Mat cvEyeImage = imread(config.personal_param_.eye_path_, CV_LOAD_IMAGE_UNCHANGED);
        if (cvEyeImage.data == nullptr) return -1;
        Mat cvModuleImage = imread(config.personal_param_.module_path_, CV_LOAD_IMAGE_UNCHANGED);
        if (cvModuleImage.data == nullptr) return -1;
        Mat cvLogoImage = imread(config.personal_param_.logo_path_, CV_LOAD_IMAGE_UNCHANGED);
    ...
    {
        Mat cvEyeImage = imread(config.personal_param_.black_eye_path_, CV_LOAD_IMAGE_UNCHANGED);
        if (cvEyeImage.data == nullptr) return -1;
        Mat cvModuleImage = imread(config.personal_param_.black_module_path_, CV_LOAD_IMAGE_UNCHANGED);
        if (cvModuleImage.data == nullptr) return -1;
        Mat cvLogoImage = imread(config.personal_param_.black_logo_path_, CV_LOAD_IMAGE_UNCHANGED);
        if (cvLogoImage.data == nullptr) return -1;
    ...
    if (config.personal_param_.mode_ == MODE_PERSONAL_WHITE)
    {
        ...
        Mat cvLogoImage(config.personal_param_.logo_image_.height_,
    ...
    else if (config.personal_param_.mode_  == MODE_PERSONAL_BLACK)
    {
        ...
        Mat cvLogoImage(config.personal_param_.black_logo_image_.height_,
...
int QBarGen::MakePersonColorQRCode(const QBAR_IMAGE& code, const QBAR_ENCODE_CONFIG& config, QBarImageInfo& buf)
{
    ...
    Mat cvHeadImage(config.personal_param_.head_image_.height_, config.personal_param_.head_image_.width_, type, config.personal_param_.head_image_.image_data_);
    
    if (config.personal_param_.head_image_.image_format_ == PIX_FMT_BGR888) cvtColor(cvHeadImage, cvHeadImage, COLOR_BGR2RGB);
    else if (config.personal_param_.head_image_.image_format_ == PIX_FMT_RGBA8888) cvtColor(cvHeadImage, cvHeadImage, COLOR_RGBA2RGB);
    else if (config.personal_param_.head_image_.image_format_ == PIX_FMT_BGRA8888) cvtColor(cvHeadImage, cvHeadImage, COLOR_BGRA2RGB);
    else if (config.personal_param_.head_image_.image_format_ == PIX_FMT_GRAY) cvtColor(cvHeadImage, cvHeadImage, COLOR_GRAY2RGB);
                                ...
                                std::vector<std::vector<int>>& domi_colors)
{
    ...
    Mat cvHeadImage(head_image.height_, head_image.width_, type, head_image.image_data_);
    if (head_image.image_format_ == PIX_FMT_BGR888) cvtColor(cvHeadImage, cvHeadImage, COLOR_BGR2RGB);
    else if (head_image.image_format_ == PIX_FMT_RGBA8888) cvtColor(cvHeadImage, cvHeadImage, COLOR_RGBA2RGB);
    else if (head_image.image_format_ == PIX_FMT_BGRA8888) cvtColor(cvHeadImage, cvHeadImage, COLOR_BGRA2RGB);
    else if (head_image.image_format_ == PIX_FMT_GRAY) cvtColor(cvHeadImage, cvHeadImage, COLOR_GRAY2RGB);
